...
def chat(intents, df):
    ...
    while True:
        ...
        if intent == 'recommend_hotel':
            price_range = input("Mumei-Bot: Please specify the price range (e.g., 100-200): ")
            district = input("Mumei-Bot: Please specify the district: ")
            amenities = input("Mumei-Bot: Please specify the amenities (comma separated): ").split(',')
